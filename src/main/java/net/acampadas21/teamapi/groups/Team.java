...
public class Team implements ConfigurationSerializable{
    ...
    public Team(String name, TeamManager tm){
        this(name, new ArrayList<Player>(), null, tm);
    }
    
    public Team(String name, ArrayList<Player> players, Player leader, TeamManager tm){
        this.name = name;
	...
	/**
	 * Sets the leader of the team only if the player belongs to it
	 * @param p Leader's player object
	 * @return true if the player has been set leader 
	 */
	public boolean setLeader(Player p){
		if(isInTeam(p)){
			leader = p;
			return true;
		}
